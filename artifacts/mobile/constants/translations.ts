export type LanguageCode = "en" | "ar";

export const LANGUAGES: { code: LanguageCode; label: string; nativeLabel: string }[] = [
  { code: "en", label: "English", nativeLabel: "English" },
  { code: "ar", label: "Arabic", nativeLabel: "العربية" },
];

export type CurrencyCode = "usd" | "sar" | "egp" | "gbp" | "aed" | "sdg" | "pkr" | "inr" | "qar" | "omr";

export interface CountryOption {
  code: string;
  name: string;
  nativeName: string;
  currency: CurrencyCode;
  symbol: string;
  icon: string;
}

export const COUNTRIES: CountryOption[] = [
  { code: "us", name: "United States", nativeName: "United States", currency: "usd", symbol: "$", icon: "flag-usa" },
  { code: "sa", name: "Saudi Arabia", nativeName: "السعودية", currency: "sar", symbol: "ر.س", icon: "flag" },
  { code: "ae", name: "United Arab Emirates", nativeName: "الإمارات", currency: "aed", symbol: "د.إ", icon: "flag" },
  { code: "gb", name: "United Kingdom", nativeName: "United Kingdom", currency: "gbp", symbol: "£", icon: "flag-variant" },
  { code: "eg", name: "Egypt", nativeName: "مصر", currency: "egp", symbol: "ج.م", icon: "flag" },
  { code: "sd", name: "Sudan", nativeName: "السودان", currency: "sdg", symbol: "ج.س", icon: "flag" },
  { code: "pk", name: "Pakistan", nativeName: "پاکستان", currency: "pkr", symbol: "Rs", icon: "flag" },
  { code: "in", name: "India", nativeName: "भारत", currency: "inr", symbol: "₹", icon: "flag" },
  { code: "qa", name: "Qatar", nativeName: "قطر", currency: "qar", symbol: "ر.ق", icon: "flag" },
  { code: "om", name: "Oman", nativeName: "عُمان", currency: "omr", symbol: "ر.ع.", icon: "flag" },
];

const COUNTRY_TO_CURRENCY: Record<string, CountryOption> = Object.fromEntries(COUNTRIES.map((c) => [c.code, c])) as Record<string, CountryOption>;

export function getCountryByCode(code?: string | null): CountryOption {
  if (code && COUNTRY_TO_CURRENCY[code]) return COUNTRY_TO_CURRENCY[code];
  return COUNTRIES[0];
}

export function getCurrencySymbolForCountry(code?: string | null): string {
  return getCountryByCode(code).symbol;
}

const CATEGORY_LABELS: Record<LanguageCode, Record<string, string>> = {
  en: {
    food: "Food", transport: "Transport", shopping: "Shopping",
    rent: "Rent", bills: "Bills", health: "Health",
    entertainment: "Entertainment", education: "Education",
    travel: "Travel", family: "Family", other: "Other",
    salary: "Salary", freelance: "Freelance", bonus: "Bonus",
    gift: "Gift", investment: "Investment", business: "Business",
    rental: "Rental", refund: "Refund",
  },
  ar: {
    food: "طعام", transport: "مواصلات", shopping: "تسوق",
    rent: "إيجار", bills: "فواتير", health: "صحة",
    entertainment: "ترفيه", education: "تعليم",
    travel: "سفر", family: "عائلة", other: "أخرى",
    salary: "راتب", freelance: "عمل حر", bonus: "مكافأة",
    gift: "هدية", investment: "استثمار", business: "عمل",
    rental: "إيجار", refund: "استرداد",
  },
};

export function getCategoryLabelForLang(category: string, lang: LanguageCode = "en"): string {
  return CATEGORY_LABELS[lang]?.[category] ?? CATEGORY_LABELS.en[category] ?? category;
}

export type TranslationKeys = typeof translations.en;

export const translations = {
  en: {
    tabHome: "Home",
    tabAnalytics: "Analytics",
    tabAdd: "Add",
    tabInsights: "Insights",
    tabBudget: "Budget",
    cancel: "Cancel",
    save: "Save",
    continue: "Continue",
    getStarted: "Get started",
    delete: "Delete",
    ok: "OK",
    seeAll: "See all",
    noData: "—",
    income: "Income",
    expenses: "Expenses",
    budget: "Budget",
    spent: "Spent",
    left: "left",
    total: "total",
    of: "of",
    vs: "vs",
    perDay: "per day",
    whatsYourName: "What's your name?",
    reportsPersonalized: "Your reports will be personalized with your name",
    yourName: "Your name",
    whatsYourCountry: "What's your country?",
    chooseYourCountry: "Choose your country",
    countrySubtitle: "We'll show the right currency based on your country",
    country: "Country",
    selectCountry: "Select country",
    whatsYourIncome: "What's your monthly income?",
    helpPlanBudget: "We'll use this to help you plan your budget",
    monthlyLimit: "Monthly spending limit?",
    setABudget: "Set a budget to track whether you're on track",
    budgetHigherThanIncome: "Budget is higher than income — are you sure?",
    whatsYourGoal: "What's your main goal?",
    tailorInsights: "We'll tailor insights and tips for you",
    goalUnderstand: "Understand my spending",
    goalUnderstandDesc: "See where your money really goes",
    goalSave: "Save more money",
    goalSaveDesc: "Build healthy saving habits",
    goalReduceFood: "Reduce food expenses",
    goalReduceFoodDesc: "Cut back on dining and groceries",
    goalControlShopping: "Control shopping",
    goalControlShoppingDesc: "Spend smarter on purchases",
    goalTrackBudget: "Track monthly budget",
    goalTrackBudgetDesc: "Stay within your spending limit",
    goodMorning: "Good morning",
    totalSpentThisMonth: "Total spent this month",
    incomeLabel: "Income",
    spentLabel: "Spent",
    usedPct: "% used",
    budgetRemaining: "remaining",
    topCategory: "Top category",
    dailyAverage: "Daily average",
    daysIn: "days in",
    moneyPersonality: "Your money personality",
    weekdays: "Weekdays",
    weekends: "Weekends",
    recentTransactions: "Recent transactions",
    noTransactionsThisMonth: "No transactions this month",
    tapToAddFirst: "Tap + to add your first one",
    pctVsLastMonth_more: "+{pct}% vs last month",
    pctVsLastMonth_less: "{pct}% vs last month",
    personalityBalanced: "Balanced Spender",
    personalityWeekend: "Weekend Spender",
    personalityFood: "Food Lover",
    personalityImpulse: "Impulse Shopper",
    personalitySmall: "Small Purchases Collector",
    personalityCareful: "Careful Planner",
    addTransaction: "Add Transaction",
    expense: "Expense",
    amountReceived: "Amount received",
    amountSpent: "Amount spent",
    incomeType: "Income type",
    category: "Category",
    customTypeName: "Custom type name",
    customTypePlaceholder: "e.g. Pet expenses, Hobbies…",
    chooseAnIcon: "Choose an icon",
    paymentMethod: "Payment method",
    cash: "Cash",
    card: "Card",
    bank: "Bank",
    wallet: "Wallet",
    date: "Date",
    noteOptional: "Note (optional)",
    noteplaceholder: "What was this for?",
    saveIncome: "Save income",
    saveExpense: "Save expense",
    savedBang: "Saved!",
    analytics: "Analytics",
    exportPDF: "Export PDF",
    day: "Day",
    month: "Month",
    year: "Year",
    spentOnThisDay: "Spent on this day",
    spentThisMonth: "Spent this month",
    spentThisYear: "Spent this year",
    earnedOnThisDay: "Earned on this day",
    earnedThisMonth: "Earned this month",
    earnedThisYear: "Earned this year",
    incomeByType: "Income by type",
    transaction: "transaction",
    transactions: "transactions",
    noExpensesInPeriod: "No expenses in this period",
    spendingByCategory: "Spending by category",
    dailySpending: "Daily spending",
    monthlySpending: "Monthly spending",
    categoryComparison: "Category comparison",
    fullBreakdown: "Full breakdown",
    noExpensesToExport: "There are no expenses in this period to export.",
    exportFailed: "Export failed",
    couldNotGeneratePDF: "Could not generate the PDF. Please try again.",
    insights: "Insights",
    behaviorAnalysis: "Behavior-based analysis of your spending",
    noInsightsYet: "No insights yet",
    addExpensesToStart: "Add some expenses to get started",
    insightTopCatTitle: "{category} is your top category",
    insightTopCatDesc: "You've spent {amount} on {category} this month — more than any other category.",
    insightSpendingUpTitle: "Spending up {pct}% this month",
    insightSpendingDownTitle: "Spending down {pct}% vs last month",
    insightSpendingUpDesc: "You've spent {curr} so far versus {prev} last month. Keep an eye on it.",
    insightSpendingDownDesc: "Great news — you're spending less than last month. You spent {curr} vs {prev} last month.",
    insightWeekendTitle: "You spend more on weekends",
    insightWeekendDesc: "Your weekend spending is {weekend} vs {weekday} on weekdays (normalized). Most of your discretionary spending happens on weekends.",
    insightSmallTitle: "Small purchases are adding up",
    insightSmallDesc: "Purchases under {threshold} totalled {total} — {pct}% of your spending. These often go unnoticed.",
    insightUnusualTitle: "{count} unusually large expense",
    insightUnusualTitlePlural: "{count} unusually large expenses",
    insightUnusualDesc: "You had {desc} significantly higher than your average: {items}.",
    insightUnusualDescOne: "an expense",
    insightUnusualDescMany: "several expenses",
    insightOverBudgetTitle: "You've exceeded your monthly budget",
    insightOverBudgetDesc: "Your spending of {spent} has gone over your {budget} budget by {over}.",
    insightNearBudgetTitle: "Approaching your monthly budget",
    insightNearBudgetDesc: "You've used {pct}% of your monthly budget. Only {left} left.",
    insightOnTrackTitle: "You're doing well this month",
    insightOnTrackDesc: "You've spent {spent} of your {budget} budget halfway through the month. Keep it up.",
    insightCatOverTitle: "{category} budget exceeded",
    insightCatOverDesc: "You've spent {spent} on {category}, which is {over} over your {limit} limit.",
    insightCatWarnTitle: "{category} is at {pct}% of budget",
    insightCatWarnDesc: "You've spent {spent} of your {limit} {category} budget.",
    insightDailyTitle: "Spending {avg} per day on average",
    insightDailyDescOk: "At this pace, you'll spend roughly {projected} this month. That's within your budget.",
    insightDailyDescOver: "At this pace, you'll spend roughly {projected} this month. This exceeds your monthly budget.",
    withinBudget: "That's within your budget.",
    exceedsBudget: "This exceeds your monthly budget.",
    monthlyBudget: "Monthly budget",
    categoryBudgets: "Category budgets",
    tapToEdit: "Tap to edit",
    addMoreBudgets: "Add more budgets",
    budgetModal: "{category} budget",
    settings: "Settings",
     support: "Support",
     contactDeveloper: "Contact developer",
     contactDeveloperSub: "Message me on Telegram for help or feedback",
     contactDeveloperUnavailable: "Telegram could not be opened on this device.",
    profile: "Profile",
    name: "Name",
    monthlyIncome: "Monthly income",
    yourData: "Your data",
    backupSize: "Backup size",
    backupRestore: "Backup & Restore",
    exportBackup: "Export backup",
    exportBackupSub: "Save a JSON file with all your data",
    importBackup: "Import backup",
    importBackupSub: "Restore data from a JSON backup file",
    autoBackup: "Auto Backup",
    autoBackupSub: "Automatically save backups to your device",
    autoBackupInterval: "Backup frequency",
    autoBackupOff: "Off",
    autoBackupDaily: "Daily",
    autoBackupWeekly: "Weekly",
    autoBackupMonthly: "Monthly",
    autoBackupLastSaved: "Last backup",
    autoBackupNever: "Never",
    autoBackupSavedTitle: "Backup saved",
    autoBackupSavedMsg: "Your data was automatically backed up as {filename}",
    autoBackupWebNote: "Auto backup saves to your device — available on iOS and Android",
    howBackupWorks: "How backup works: ",
    howBackupWorksText: "Export saves a .json file to your device (or lets you share it via email, Drive, etc.). To restore, open the app after reinstalling and tap Import, then pick the same file.",
    dangerZone: "Danger zone",
    clearAllData: "Clear all data",
    clearAllDataSub: "Permanently delete everything and start over",
    clearAllDataConfirmTitle: "Clear all data",
    clearAllDataConfirmMsg: "This will permanently delete ALL your expenses, profile, and settings. This cannot be undone.\n\nWe recommend exporting a backup first.",
    deleteEverything: "Delete everything",
    importConfirmTitle: "Import backup",
    importConfirmMsg: "This will replace ALL your current data with the contents of the backup file. Continue?",
    importBtnLabel: "Import",
    importSuccessTitle: "Import successful",
    importSuccessMsg: "Restored {count} transaction from backup dated {date}.",
    importSuccessMsgPlural: "Restored {count} transactions from backup dated {date}.",
    exportFailedTitle: "Export failed",
    importFailedTitle: "Import failed",
    saveChanges: "Save changes",
    language: "Language",
    appLanguage: "App language",
    restartRequiredTitle: "Restart required",
    restartRequiredMsg: "The app will restart to apply the language and layout change.",
    version: "Rasheed v1.0.0",
    smartBudgetTitle: "Smart Budget Suggestions",
    smartBudgetSub: "Local analysis · Last {months} {monthWord} · No internet",
    smartBudgetMonthSingular: "month",
    smartBudgetMonthPlural: "months",
    smartBudgetOffline: "Offline",
    smartBudgetPotentialSaving: "Potential monthly saving",
    smartBudgetPerMonth: "per month",
    smartBudgetSuggestionsLabel: "suggestions",
    smartBudgetDataMonths: "data months",
    smartBudgetIncomeCapped: "Suggestions adjusted to fit your monthly income",
    smartBudgetCurrentBudget: "Current budget",
    smartBudgetRecommended: "Recommended",
    smartBudgetCurrentBar: "Current",
    smartBudgetRecommendedBar: "Suggested",
    smartBudgetSavePill: "Save {symbol}{amount}/mo",
    smartBudgetIncreasePill: "Increase {symbol}{amount}/mo",
    smartBudgetDataInfo: "Based on {months} {monthWord} · Avg spending: {symbol}{avg}",
    smartBudgetApply: "Apply suggestion",
    smartBudgetApplied: "Applied",
    smartBudgetApplyAll: "Apply all suggestions ({count})",
    smartBudgetAllApplied: "All suggestions applied",
    smartBudgetOptimizedTitle: "Your budget is already optimized",
    smartBudgetOptimizedSub: "No adjustments suggested right now — keep logging expenses to improve analysis accuracy",
    smartBudgetConfidenceHigh: "High confidence",
    smartBudgetConfidenceMedium: "Med. confidence",
    smartBudgetConfidenceLow: "Limited data",
    securityQuestion: "Security Question",
    securityQuestionSub: "Set a question to recover your PIN",
    selectQuestion: "Select a question",
    yourAnswer: "Your answer",
    saveQuestion: "Save",
    wrongAnswer: "Wrong answer — try again",
    forgotPIN: "Forgot PIN?",
    answerToReset: "Answer to reset your PIN",
    verifyAnswer: "Verify",
    resetPINSuccess: "PIN has been reset",
    notifications: "Notifications",
    budgetAlerts: "Budget Alerts",
    budgetAlertsSub: "Alert when you reach 80% or 100% of budget",
    dailyReminder: "Daily Reminder",
    dailyReminderSub: "Remind you to log your expenses",
    reminderTime: "Reminder time",
    notifPermissionDenied: "Enable notifications in your device settings",
    budgetAlert80Title: "Budget Warning",
    budgetAlert80Body: "You've used 80% of your monthly budget ({amount} remaining)",
    budgetAlert100Title: "Budget Exceeded",
    budgetAlert100Body: "You've gone over your monthly budget by {amount}",
    dailyReminderTitle: "Time to log your expenses!",
    dailyReminderBody: "Keep your budget on track — add today's spending",
    privacy: "Privacy & Security",
    pinLock: "PIN Lock",
    pinLockSub: "Require a 4-digit PIN to open the app",
    biometricAuth: "Biometric",
    biometricAuthSub: "Fingerprint or Face ID",
    changePIN: "Change PIN",
    setupPIN: "Set up PIN",
    setupPINSub: "Choose a 4-digit PIN to secure your data",
    confirmPIN: "Confirm PIN",
    confirmPINSub: "Re-enter the same PIN to confirm",
    pinMismatch: "PINs don't match — try again",
    wrongPIN: "Incorrect PIN",
    enterPIN: "Enter your PIN",
    tooManyAttempts: "Too many attempts — locked",
    attemptsLeft: "attempts left",
    attemptsRemaining: "Try again in {count}",
    pinDisableConfirm: "Disable PIN?",
    pinDisableMsg: "Your app will no longer require a PIN to open.",
    disable: "Disable",
    pinSetSuccess: "PIN set successfully",
    allTransactions: "All Transactions",
    searchTransactions: "Search by note, category, or amount…",
    filterAll: "All time",
    filterToday: "Today",
    filterThisWeek: "This week",
    filterThisMonth: "This month",
    filterThisYear: "This year",
    allTypes: "All",
    allCategories: "All categories",
    sortNewest: "Newest",
    sortOldest: "Oldest",
    sortLargest: "Largest",
    sortSmallest: "Smallest",
    sort_newest: "Newest",
    sort_oldest: "Oldest",
    sort_largest: "Largest",
    sort_smallest: "Smallest",
    noResults: "No matching transactions",
    noResultsSub: "Try a different search or filter",
    clearFilters: "Clear",
    report: "Monthly Report",
    reportSub: "Summary for {month}",
    exportReport: "Export PDF",
    totalSpent: "Total Spent",
    avgPerDay: "Avg / Day",
    transactions2: "Transactions",
    vsLastMonth: "vs last month",
    topSpending: "Top spending categories",
    unusualSpend: "Unusual spending",
    noData2: "No data this month",
    noUnusual: "No unusually high transactions this month",
  },
  ar: { tabHome: "الرئيسية", tabAnalytics: "التحليلات", tabAdd: "إضافة", tabInsights: "رؤى", tabBudget: "الميزانية", cancel: "إلغاء", save: "حفظ", continue: "متابعة", getStarted: "ابدأ الآن", delete: "حذف", ok: "موافق", seeAll: "عرض الكل", noData: "—", income: "الدخل", expenses: "المصروفات", budget: "الميزانية", spent: "المصروف", left: "متبقي", total: "الإجمالي", of: "من", vs: "مقارنةً بـ", perDay: "يوميًا", whatsYourName: "ما اسمك؟", reportsPersonalized: "سيتم تخصيص التقارير باسمك", yourName: "اسمك", whatsYourCountry: "ما بلدك؟", chooseYourCountry: "اختر بلدك", countrySubtitle: "سنعرض العملة المناسبة حسب بلدك", country: "البلد", selectCountry: "اختر البلد", whatsYourIncome: "ما دخلك الشهري؟", helpPlanBudget: "سنستخدمه لمساعدتك في تخطيط ميزانيتك", monthlyLimit: "حد الإنفاق الشهري؟", setABudget: "ضع ميزانية لمعرفة إن كنت على المسار الصحيح", budgetHigherThanIncome: "الميزانية أعلى من الدخل — هل أنت متأكد؟", whatsYourGoal: "ما هدفك الرئيسي؟", tailorInsights: "سنخصص لك الرؤى والنصائح", goalUnderstand: "فهم إنفاقي", goalUnderstandDesc: "اعرف أين يذهب مالك بالفعل", goalSave: "ادخار المزيد", goalSaveDesc: "ابنِ عادات ادخار صحية", goalReduceFood: "تقليل مصروف الطعام", goalReduceFoodDesc: "خفف من المطاعم والبقالة", goalControlShopping: "التحكم في التسوق", goalControlShoppingDesc: "أنفق بذكاء على المشتريات", goalTrackBudget: "متابعة الميزانية الشهرية", goalTrackBudgetDesc: "ابقَ ضمن حد الإنفاق", goodMorning: "صباح الخير", totalSpentThisMonth: "إجمالي ما تم إنفاقه هذا الشهر", incomeLabel: "الدخل", spentLabel: "المصروف", usedPct: "% مستخدم", budgetRemaining: "متبقي", topCategory: "أعلى فئة", dailyAverage: "المتوسط اليومي", daysIn: "أيام في", moneyPersonality: "شخصية الإنفاق لديك", weekdays: "أيام الأسبوع", weekends: "نهاية الأسبوع", recentTransactions: "آخر العمليات", noTransactionsThisMonth: "لا توجد عمليات هذا الشهر", tapToAddFirst: "اضغط + لإضافة أول عملية", pctVsLastMonth_more: "+{pct}% مقارنةً بالشهر الماضي", pctVsLastMonth_less: "{pct}% مقارنةً بالشهر الماضي", personalityBalanced: "منفق متوازن", personalityWeekend: "منفق عطلة نهاية الأسبوع", personalityFood: "محب للطعام", personalityImpulse: "مشتري اندفاعي", personalitySmall: "جامع المشتريات الصغيرة", personalityCareful: "مخطط حذر", addTransaction: "إضافة عملية", expense: "مصروف", amountReceived: "المبلغ المستلم", amountSpent: "المبلغ المصروف", incomeType: "نوع الدخل", category: "الفئة", customTypeName: "اسم مخصص", customTypePlaceholder: "مثال: مصروفات الحيوانات، الهوايات…", chooseAnIcon: "اختر رمزًا", paymentMethod: "طريقة الدفع", cash: "نقدًا", card: "بطاقة", bank: "بنك", wallet: "محفظة", date: "التاريخ", noteOptional: "ملاحظة (اختياري)", noteplaceholder: "لِمَ كان هذا؟", saveIncome: "حفظ الدخل", saveExpense: "حفظ المصروف", savedBang: "تم الحفظ!", analytics: "التحليلات", exportPDF: "تصدير PDF", day: "يوم", month: "شهر", year: "سنة", spentOnThisDay: "تم الإنفاق في هذا اليوم", spentThisMonth: "تم الإنفاق هذا الشهر", spentThisYear: "تم الإنفاق هذا العام", transaction: "عملية", transactions: "عمليات", noExpensesInPeriod: "لا توجد مصروفات في هذه الفترة", spendingByCategory: "الإنفاق حسب الفئة", dailySpending: "الإنفاق اليومي", monthlySpending: "الإنفاق الشهري", categoryComparison: "مقارنة الفئات", fullBreakdown: "تفصيل كامل", noExpensesToExport: "لا توجد مصروفات في هذه الفترة للتصدير.", exportFailed: "فشل التصدير", couldNotGeneratePDF: "تعذر إنشاء ملف PDF. حاول مرة أخرى.", insights: "رؤى", behaviorAnalysis: "تحليل سلوك إنفاقك", noInsightsYet: "لا توجد رؤى بعد", addExpensesToStart: "أضف بعض المصروفات للبدء", insightTopCatTitle: "{category} هي فئتك الأعلى", insightTopCatDesc: "أنفقت {amount} على {category} هذا الشهر — أكثر من أي فئة أخرى.", insightSpendingUpTitle: "الإنفاق ارتفع {pct}% هذا الشهر", insightSpendingDownTitle: "الإنفاق انخفض {pct}% مقارنة بالشهر الماضي", insightSpendingUpDesc: "أنفقت {curr} حتى الآن مقابل {prev} الشهر الماضي. انتبه لذلك.", insightSpendingDownDesc: "خبر رائع — إنفاقك أقل من الشهر الماضي. أنفقت {curr} مقابل {prev} الشهر الماضي.", insightWeekendTitle: "تنفق أكثر في عطلة نهاية الأسبوع", insightWeekendDesc: "إنفاق عطلة نهاية الأسبوع هو {weekend} مقابل {weekday} في أيام الأسبوع. معظم الإنفاق الاختياري يحدث في نهاية الأسبوع.", insightSmallTitle: "المشتريات الصغيرة تتجمع", insightSmallDesc: "المشتريات تحت {threshold} بلغت {total} — {pct}% من إنفاقك. غالبًا ما تمر دون ملاحظة.", insightUnusualTitle: "{count} مصروف كبير غير معتاد", insightUnusualTitlePlural: "{count} مصروفات كبيرة غير معتادة", insightUnusualDesc: "لديك {desc} أعلى بكثير من المتوسط: {items}.", insightUnusualDescOne: "مصروف واحد", insightUnusualDescMany: "عدة مصروفات", insightOverBudgetTitle: "تجاوزت ميزانيتك الشهرية", insightOverBudgetDesc: "إنفاقك {spent} تجاوز ميزانيتك {budget} بمقدار {over}.", insightNearBudgetTitle: "أنت تقترب من الميزانية الشهرية", insightNearBudgetDesc: "استخدمت {pct}% من ميزانيتك الشهرية. بقي {left} فقط.", insightOnTrackTitle: "أداؤك جيد هذا الشهر", insightOnTrackDesc: "أنفقت {spent} من ميزانية {budget} في منتصف الشهر. استمر.", insightCatOverTitle: "تجاوزت ميزانية {category}", insightCatOverDesc: "أنفقت {spent} على {category}، وهذا يتجاوز حد {limit} بمقدار {over}.", insightCatWarnTitle: "{category} عند {pct}% من الميزانية", insightCatWarnDesc: "أنفقت {spent} من ميزانية {limit} الخاصة بـ {category}.", insightDailyTitle: "تنفق {avg} يوميًا في المتوسط", insightDailyDescOk: "بهذا المعدل، ستنفق تقريبًا {projected} هذا الشهر. هذا ضمن ميزانيتك.", insightDailyDescOver: "بهذا المعدل، ستنفق تقريبًا {projected} هذا الشهر. هذا يتجاوز ميزانيتك الشهرية.", withinBudget: "هذا ضمن ميزانيتك.", exceedsBudget: "هذا يتجاوز ميزانيتك الشهرية.", monthlyBudget: "الميزانية الشهرية", categoryBudgets: "ميزانيات الفئات", tapToEdit: "اضغط للتعديل", addMoreBudgets: "إضافة ميزانيات أخرى", budgetModal: "ميزانية {category}", settings: "الإعدادات", profile: "الملف الشخصي", name: "الاسم", monthlyIncome: "الدخل الشهري", yourData: "بياناتك", backupSize: "حجم النسخة الاحتياطية", backupRestore: "نسخ احتياطي واستعادة", exportBackup: "تصدير النسخة الاحتياطية", exportBackupSub: "حفظ ملف JSON يحتوي على كل بياناتك", importBackup: "استيراد نسخة احتياطية", importBackupSub: "استعادة البيانات من ملف JSON", howBackupWorks: "كيف تعمل النسخة الاحتياطية: ", howBackupWorksText: "يتم حفظ ملف .json على جهازك (أو يمكنك مشاركته عبر البريد أو Drive). للاستعادة، افتح التطبيق بعد إعادة التثبيت واضغط استيراد ثم اختر الملف نفسه.", dangerZone: "منطقة خطرة", clearAllData: "مسح كل البيانات", clearAllDataSub: "حذف كل شيء نهائيًا والبدء من جديد", clearAllDataConfirmTitle: "مسح كل البيانات", clearAllDataConfirmMsg: "سيؤدي هذا إلى حذف جميع المصروفات والملف الشخصي والإعدادات نهائيًا. لا يمكن التراجع عن ذلك.\n\nننصح بتصدير نسخة احتياطية أولًا.", deleteEverything: "حذف الكل", importConfirmTitle: "استيراد نسخة احتياطية", importConfirmMsg: "سيستبدل هذا كل بياناتك الحالية بمحتويات ملف النسخة الاحتياطية. هل تريد المتابعة؟", importBtnLabel: "استيراد", importSuccessTitle: "تم الاستيراد بنجاح", importSuccessMsg: "تمت استعادة {count} عملية من نسخة بتاريخ {date}.", importSuccessMsgPlural: "تمت استعادة {count} عمليات من نسخة بتاريخ {date}.", exportFailedTitle: "فشل التصدير", importFailedTitle: "فشل الاستيراد", autoBackup: "النسخ الاحتياطي التلقائي", autoBackupSub: "حفظ النسخ الاحتياطية تلقائيًا على جهازك", autoBackupInterval: "تكرار النسخ الاحتياطي", autoBackupOff: "إيقاف", autoBackupDaily: "يوميًا", autoBackupWeekly: "أسبوعيًا", autoBackupMonthly: "شهريًا", autoBackupLastSaved: "آخر نسخة احتياطية", autoBackupNever: "لم يتم بعد", autoBackupSavedTitle: "تم حفظ النسخة الاحتياطية", autoBackupSavedMsg: "تم حفظ بياناتك تلقائيًا بصيغة {filename}", autoBackupWebNote: "النسخ الاحتياطي التلقائي متاح على iOS و Android فقط", saveChanges: "حفظ التغييرات", language: "اللغة", appLanguage: "لغة التطبيق", restartRequiredTitle: "إعادة التشغيل مطلوبة", restartRequiredMsg: "سيُعاد تشغيل التطبيق لتطبيق تغييرات اللغة والتخطيط.", version: "Intensive v1.0.0", report: "التقرير الشهري", reportSub: "ملخص {month}", exportReport: "تصدير PDF", totalSpent: "إجمالي المصروف", avgPerDay: "المتوسط / يوم", transactions2: "العمليات", vsLastMonth: "مقارنة بالشهر الماضي", topSpending: "أعلى فئات الإنفاق", unusualSpend: "إنفاق غير معتاد", noData2: "لا توجد بيانات هذا الشهر", noUnusual: "لا توجد عمليات كبيرة غير معتادة هذا الشهر", allTransactions: "كل العمليات", searchTransactions: "بحث بالملاحظة أو الفئة أو المبلغ…", filterAll: "كل الأوقات", filterToday: "اليوم", filterThisWeek: "هذا الأسبوع", filterThisMonth: "هذا الشهر", filterThisYear: "هذا العام", allTypes: "الكل", allCategories: "كل الفئات", sortNewest: "الأحدث", sortOldest: "الأقدم", sortLargest: "الأكبر", sortSmallest: "الأصغر", sort_newest: "الأحدث", sort_oldest: "الأقدم", sort_largest: "الأكبر", sort_smallest: "الأصغر", noResults: "لا توجد عمليات مطابقة", noResultsSub: "جرب بحثًا أو فلترًا مختلفًا", clearFilters: "مسح" }, smartBudgetTitle: "اقتراحات الميزانية الذكية", smartBudgetSub: "تحليل محلي · آخر {months} {monthWord} · بدون إنترنت", smartBudgetMonthSingular: "شهر", smartBudgetMonthPlural: "أشهر", smartBudgetOffline: "أوفلاين", smartBudgetPotentialSaving: "إجمالي التوفير المحتمل", smartBudgetPerMonth: "شهرياً", smartBudgetSuggestionsLabel: "اقتراح", smartBudgetDataMonths: "أشهر بيانات", smartBudgetIncomeCapped: "تم تعديل الاقتراحات لتناسب دخلك الشهري", smartBudgetCurrentBudget: "الميزانية الحالية", smartBudgetRecommended: "الموصى بها", smartBudgetCurrentBar: "الحالي", smartBudgetRecommendedBar: "الموصى", smartBudgetSavePill: "وفّر {symbol}{amount} شهرياً", smartBudgetIncreasePill: "زيادة {symbol}{amount} شهرياً", smartBudgetDataInfo: "بناءً على {months} {monthWord} · متوسط إنفاقك: {symbol}{avg}", smartBudgetApply: "تطبيق الاقتراح", smartBudgetApplied: "تم التطبيق", smartBudgetApplyAll: "تطبيق جميع الاقتراحات ({count})", smartBudgetAllApplied: "تم تطبيق جميع الاقتراحات", smartBudgetOptimizedTitle: "ميزانيتك محسّنة بالفعل", smartBudgetOptimizedSub: "لا توجد تعديلات مقترحة حالياً — استمر في تسجيل مصروفاتك لتحسين دقة التحليل", smartBudgetConfidenceHigh: "ثقة عالية", smartBudgetConfidenceMedium: "ثقة متوسطة", smartBudgetConfidenceLow: "بيانات محدودة", privacy: "الخصوصية والأمان", pinLock: "قفل PIN", pinLockSub: "طلب رمز PIN لفتح التطبيق", biometricAuth: "بصمة الإصبع", biometricAuthSub: "بصمة الإصبع أو Face ID", changePIN: "تغيير PIN", setupPIN: "إعداد PIN", setupPINSub: "اختر رمز PIN مكون من 4 أرقام", confirmPIN: "تأكيد PIN", confirmPINSub: "أعد إدخال نفس الرمز للتأكيد", pinMismatch: "الرمزان غير متطابقان — حاول مرة أخرى", wrongPIN: "رمز PIN غير صحيح", enterPIN: "أدخل رمز PIN", tooManyAttempts: "محاولات كثيرة جداً — مقفل", attemptsLeft: "محاولات متبقية", attemptsRemaining: "حاول مرة أخرى خلال {count}", pinDisableConfirm: "إلغاء PIN؟", pinDisableMsg: "لن يحتاج التطبيق إلى رمز PIN لفتحه بعد الآن.", disable: "إلغاء", pinSetSuccess: "تم تعيين PIN بنجاح", securityQuestion: "سؤال الأمان", securityQuestionSub: "اضبط سؤالاً لاسترداد رمز PIN", selectQuestion: "اختر سؤالاً", yourAnswer: "إجابتك", saveQuestion: "حفظ", wrongAnswer: "إجابة خاطئة — حاول مرة أخرى", forgotPIN: "نسيت PIN؟", answerToReset: "أجب لإعادة تعيين PIN", verifyAnswer: "تحقق", resetPINSuccess: "تم إعادة تعيين PIN", notifications: "الإشعارات", budgetAlerts: "تنبيهات الميزانية", budgetAlertsSub: "إشعار عند الوصول لـ 80% أو 100% من الميزانية", dailyReminder: "تذكير يومي", dailyReminderSub: "تذكير بتسجيل مصاريفك اليومية", reminderTime: "وقت التذكير", notifPermissionDenied: "فعّل الإشعارات من إعدادات الجهاز", budgetAlert80Title: "تحذير الميزانية", budgetAlert80Body: "لقد استهلكت 80% من ميزانيتك ({amount} متبقية)", budgetAlert100Title: "تجاوزت الميزانية", budgetAlert100Body: "لقد تجاوزت ميزانيتك الشهرية بـ {amount}", dailyReminderTitle: "حان وقت تسجيل مصاريفك!", dailyReminderBody: "تابع ميزانيتك — أضف مصاريف اليوم",
} as const;

Object.assign(translations.ar, {
  support: "الدعم",
  contactDeveloper: "تواصل مع المطوّر",
  contactDeveloperSub: "راسلني عبر Telegram للمساعدة أو مشاركة ملاحظاتك",
  contactDeveloperUnavailable: "تعذر فتح Telegram على هذا الجهاز.",
});

export function interpolate(template: string, vars: Record<string, string | number>): string {
  return template.replace(/\{(\w+)\}/g, (_, key) => String(vars[key] ?? `{${key}}`));
}
